...
class Q1 {
    ...
    int numStates, numTerminal, numNT;
    String[] terminalNames, nonTerminalNames;
    ArrayList<String>[] I;
        ...
        for (int i = 0; i < numStates; i++) {
            ...
            if (group[i] == 0) {
                ...
                for (int j = i + 1; j < numStates; j++) {
                    ...
                    if (core[i].equals(core[j])) group[j] = gCnt;
                }
            }
        }
        ...
        for (int i = 0; i < numStates; i++) {
            ...
            for (int j = i + 1; j < numStates; j++) {
                if (group[i] == group[j] && i != j) {
                    for (String item : I[j]) if (!I[i].contains(item)) I[i].add(item);
                    I[j].clear();
                    core[j] = "";
                }
            }
    ...
    public void makeLALR() {
        ...
        for (int s = 0; s < numStates; s++) {
            ...
            for (int t = 0; t < numTerminal; t++) {
                ...
                if (LALR_ACTION[g][t] == null) {
                    ...
                    if ((cur.startsWith("s") && aNew.startsWith("r")) ||
                            (cur.startsWith("r") && aNew.startsWith("s"))) shiftReduceConflict = true;
                    if (cur.startsWith("r") && aNew.startsWith("r") && !cur.equals(aNew))
            ...
            for (int n = 0; n < numNT; n++) {
                ...
                if (go == -1) continue;
                int ng = group[go] - 1;
                if (LALR_GOTO[g][n] == -1) LALR_GOTO[g][n] = ng;
            }
        }
    }
    ...
    String getMergedStateID(int g) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < numStates; i++) if (group[i] - 1 == g) sb.append(i);
        return sb.toString();
    }
    ...
    String beautify(String action) {
        ...
        if (action.startsWith("s")) {
            ...
            return "s" + getMergedStateID(next);
        }
        return action;
    }
    ...
    public void printLALR() {
        ...
        for (String term : terminalNames) System.out.printf("%-10s", term);
        for (String nt : nonTerminalNames) System.out.printf("%-10s", nt);
        System.out.println();

        for (int g = 0; g < mg; g++) {
            System.out.printf("%-10s", getMergedStateID(g));
            for (int t = 0; t < numTerminal; t++)
                System.out.printf("%-10s", beautify(LALR_ACTION[g][t]));
            for (int n = 0; n < numNT; n++)
                System.out.printf("%-10s", LALR_GOTO[g][n] == -1 ? "-" : getMergedStateID(LALR_GOTO[g][n]));
            System.out.println();
        }
